...
# Save Data
base_dir = "C:/Users/23943/Desktop/RF_depth_10_Normalized"
max_depth = 8
...
if training_flag:
    ...
    if not os.path.isfile(filename):
        ...
        for joint_idx in range(7):
            ...
            # # print max_depth for each joint
            # 输出每个joint的max_depth
            # max_depths = [estimator.tree_.max_depth for estimator in rf_model.estimators_]
            # print(f"\nJoint {joint_idx + 1} - Maximum depth of each tree: {max_depths}")
            # print(f"Joint {joint_idx + 1} - Overall Maximum Depth: {max(max_depths)}")
            ...
            if visualize:
                ...
                plt.legend()
                plt.grid(True)
                # plt.savefig(os.path.join(base_dir, f'joint_{joint_idx+1}_depth_{max_depth}.png'))
                plt.show()
    ...
    for goal_position in goal_positions:
        ...
        if visualize:
            ...
            info_text = (
                ...
                # f"Second Last Predicted Position: ({second_last_predicted_pos[0]:.4f}, {second_last_predicted_pos[1]:.4f}, {second_last_predicted_pos[2]:.4f})\n"
                f"Position Error: {position_error:.4f} m"
            )
            plt.gcf().text(0.5, 0.04, info_text, fontsize=10, color='purple', 
